package data
{
	...
	public class DBConnect extends EventDispatcher
	{
		...
		//Stats DB
		private var _connStatsDB:SQLConnection;
		private var _dataLoadedStatsDB:Boolean = false;
		private var _playerId:int = -1;
		private var _score:int = -1;
		
		public function DBConnect()
		{
			addEventListener(EVENT_MAIN_DB_CREATED, function() {
					connectToStatsDB();
				});
		...
		private function connectToStatsDB():void
		{
		...
		private function openStatsDB($dbDir:File):void
		{
			var dbFile:File = $dbDir.resolvePath(STATS_DB_NAME);
			
			_connStatsDB = new SQLConnection();
			_connStatsDB.addEventListener(SQLEvent.OPEN, onStatsDBOpen);
			_connStatsDB.addEventListener(SQLErrorEvent.ERROR, onDBError);
			_connStatsDB.openAsync(dbFile);
		}
		
		private function onStatsDBOpen($e:SQLEvent):void
		{
			var stat:SQLStatement = new SQLStatement();
			stat.sqlConnection = _connStatsDB;
			stat.text = "CREATE TABLE IF NOT EXISTS PlayerStats (id INTEGER PRIMARY KEY AUTOINCREMENT, playerId INTEGER, score INTEGER)";
			stat.execute();
		}
		
		
		public function savePlayerScore($playerId:int, $score:int):void
		{
			var q:SQLStatement = new SQLStatement();
			q.sqlConnection = _connStatsDB;
			q.clearParameters();
			q.text = "INSERT  INTO 'PlayerStats' VALUES( NULL, :playerId, :score )";
		...
		public function getPlayerScore($playerId:int):void
		{
			var q:SQLStatement = new SQLStatement();
			q.sqlConnection = _connStatsDB;
			q.text = "SELECT * FROM 'PlayerStats' WHERE playerId=" + $playerId;
		...
		public function selectAllPlayers($e:SQLResult = null):void
		{
			...
			dispatchEvent(new Event(EVENT_MAIN_DB_CREATED));
		}
		...
		private function onPlayersSelected($e:SQLEvent):void
		{
			...
			var stat:SQLStatement = new SQLStatement();
			stat.sqlConnection = _connMainDB;
			stat.text = "CREATE TABLE IF NOT EXISTS Stats (id INTEGER PRIMARY KEY AUTOINCREMENT, " + "userId INTEGER, score INTEGER, wpm INTEGER, accuracy REAL, weakHand TEXT, weakFinger TEXT, " + "date INTEGER, indexErrCount INTEGER, middleErrCount INTEGER, ringErrCount INTEGER, " + "pinkyErrCount INTEGER, lHandErrCount INTEGER, rHandErrCount INTEGER)";
			stat.execute();
		...
		private function parsePlayerData($data:Object):void
		{
			...
			{
				_allPlayers.push(new Player(pData.id, pData.nick, pData.name, pData.lname));
			}
