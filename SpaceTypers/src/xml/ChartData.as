package xml 
{
	...
	public class ChartData extends EventDispatcher
	{
		...
		public function ChartData(...rest)
		{
			//Main node
			_data = new XML(< {_LINE_CHART} />);
			...
			for (var i:int = 0; i < rest.length -1; i++ )
			{
				...
				serie.@name = rest[i +  1][0];
				
				series.appendChild(serie);
			}
			_data.appendChild(series);
			...
			//}
			//
			//_data.appendChild(periods);
			
			
			var periods:XML = new XML(< {_PERIODS} />);
			var period:XML;
			var val:XML;
			for (var k:int = 0; k < rest[0].length; k++ )
			{
				period = new XML(< {_PERIOD} />);
				period.@name = "'"  + rest[0][k] + "'";
				
				for (var j:int = 1; j < rest.length;  j++ )
				{
					val = new XML(<v>{rest[j][k+1]}</v>);
					period.appendChild(val);
				}
			...
			_data.appendChild(periods);
			//trace('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', periods.toString());
			//var f:FileReference = new FileReference;
			//f.addEventListener(Event.COMPLETE, onFileSaved);
			//f.save( _data, "line-charta.xml" );
		}
